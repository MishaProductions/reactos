...
static BOOL add_cookie( struct session *session, struct cookie *cookie, WCHAR *domain_name, WCHAR *path )
{
    ...
    if (!(cookie->path = _wcsdup( path ))) return FALSE;
...
BOOL set_cookies( struct request *request, const WCHAR *cookies )
{
    ...
    if (!cookie_domain && !(cookie_domain = _wcsdup( request->connect->servername ))) goto end;
    if (!cookie_path && !(cookie_path = _wcsdup( request->path ))) goto end;
